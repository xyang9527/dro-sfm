...
class ProjectionInputPose(nn.Module):
    ...
    def forward(self, pose, cost):
        ...
        return torch.cat([out_p, pose.view(bs, 6, 1, 1).repeat(1, 1, h, w)], dim=1)
...
class BasicUpdateBlockDepth(nn.Module):
    ...
    def forward(self, net, cost_func, inv_depth, context, seq_len=4, scale_func=None):
        inv_depth_list = [] 
        mask_list = []
        for i in range(seq_len):
            # TODO detach()
            # inv_depth = inv_depth.detach()
            input_features = self.encoder(inv_depth, cost_func(scale_func(inv_depth)[0]))
