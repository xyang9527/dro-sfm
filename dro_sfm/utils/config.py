...
from dro_sfm.utils.types import is_cfg, is_list
from dro_sfm.utils.misc import make_list
from dro_sfm.utils.load import load_class, backwards_state_dict
...
def set_checkpoint(config):
    ...
    if config.checkpoint.filepath is not '':
        ...
        config.checkpoint.filepath = os.path.join(
            ...
            config.model.name + "_" + config.model.depth_net.name + "_" +
            config.model.depth_net.version + "_" +
...
def parse_train_file(file, config):
    ...
    # If it's a .ckpt checkpoint file
        ...
        if config is None:
            ...
            checkpoint = torch.load(file, map_location='cpu')
            cfg_default = 'configs/default_config'
            config = parse_train_config(cfg_default, config)
            checkpoint['file'] = file
            return config, checkpoint 
    # We have a problem
    else:
